 ...
 */
/* 
* Test case for UNIREP (uncorrected, box, GG, HF), exact, unconditional power
...
DO ROW = 1 to NROW(HOLDIN);
  ...
  DO IPROB=1 TO NCOL(PROBLIST);
	...
	DO TESTI=1 to NCOL(TEST_LIST);
		EPOW=BASEU4("E",TEST_LIST[TESTI],ESSENCEF,REPN,CF,CG,VARG,THETAD,SIGSTAR,
             ALPHA,MAXPDIFF);
		POWER_ROW = POWER_ROW || EPOW[1,1];
	END;
