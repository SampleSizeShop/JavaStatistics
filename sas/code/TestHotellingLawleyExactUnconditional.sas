 ...
 */
/* 
* Test case for Hotelling Lawley, exact, unconditional power
...
DO ROW = 1 to NROW(HOLDIN);
  ...
  DO IPROB=1 TO NCOL(PROBLIST);
	/* approximate unconditional power */
  	EPOWUNH1=BASEU4("E",H_OR_U,ESSENCEF,REPN,CF,CG,VARG,THETAD,SIGSTAR,
             ALPHA,MAXPDIFF);
    POWER=POWER//(DELTA||N||EPOWUNH1[1,1]);
   END;
END;
