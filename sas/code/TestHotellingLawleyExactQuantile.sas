 ...
 */
/* 
* Test case for Hotelling Lawley, exact, median quantile power
...
DO ROW = 1 to NROW(HOLDIN);
  ...
  DO IPROB=1 TO NCOL(PROBLIST);
    ...
    RUN BASEQ1("E",H_OR_U,ESSENCEF,REPN,CF,CG,VARG,THETAD,SIGSTAR,
                 PROBLIST[IPROB],MAXITER,MAXPDIFF, ITERE,QA,FQA);
	/* Quantile power */
	APOWERAQ=APOWH2(A,B,N,RANKX,ALPHA,QA);
    POWER=POWER//(DELTA||N||QA||APOWERAQ);
   END;
END;

NAMES = {'Beta-Scale' 'Total N' 'Non-centrality' 'HLT Power'};
PRINT POWER[COLNAME=NAMES];

/* write to XML file */
filename out "&OUTPUT_DATA_DIRECTORY\TestHotellingLawleyExactQuantile.xml"; 
file out;
	put "<powerList>";
	do i=1 to NROW(POWER);
		...
		put (POWER[i,2]) @;
		put "' powerMethod='quantile' quantile='0.5' />";
	end;
	put "</powerList>";
closefile out;
